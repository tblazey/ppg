...
def count_int(time, counts, limits):
    ...
    int_msk = np.logical_and(time >= limits[0],
                             time <= limits[1])
...
def comp_img_dim(hdr_one, hdr_two, dim_end=None):
    ...
    if hdr_one.shape[0:dim_end] != hdr_two.shape[0:dim_end]:
        ...
        msg = '%s and %s do not have the same dimensions'%(name_one, name_two)
        raise ValueError(msg)
...
def iida_oxy_aif(aif, delta=20, prod=0.0012):
    ...
    aif_func = interp.interp1d(aif.time, aif.cnt, kind='cubic',
...
def load_pet(pet_path, h_life,
             censor_path=None, msk_path=None, limit=None,
             scale=1.0/0.8657, time_path=None, vol_path=None):
    """
    Loads in PET data
    ...
    if time_path is not None:
        ...
        if pet_time.shape[0] < pet_hdr.shape[3]:
            raise ValueError('Timing file %s does not have enough data points'%(time_path))
...
def natural_spline_basis(x, knots, dot=None):
    ...
    offset = (np.power(np.maximum(0, x - knots[n_k-2]), 3)
              - last_spline) / (knots[-1] - knots[n_k-2])
    ...
    if dot is not None:
        ...
        offset_d = (dot_coef * np.power(np.maximum(0, x - knots[n_k-2]), dot_power)
                    - last_spline_d) / (knots[-1] - knots[n_k-2])

    #Compute cubic terms
    for i in range(0, n_k-2):

        #Add spline to basis
        basis[:, i+2] = ((np.power(np.maximum(0, x - knots[i]), 3)
                        - last_spline) / (knots[-1] - knots[i])) - offset

        #Add derivative term if necessary
        if dot is not None:
            basis_d[:, i+2] = ((dot_coef * np.power(np.maximum(0, x - knots[i]), dot_power)
                              - last_spline_d) / (knots[-1] - knots[i])) - offset_d
...
def prep_model(aif_path, pet_path, time_path, msk_path,
               vol_path, scale, limit, censor_path, h_life,
               img_paths=None, unif=False):
    """
    ...
    (pet_hdr, pet_mskt, msk_data,
     msk_hdr, mean_tac_mskt) = load_pet(pet_path, h_life,
                                        censor_path=censor_path,
                                        msk_path=msk_path, limit=limit,
                                        scale=scale, time_path=time_path,
                                        vol_path=vol_path)
                                       
    #Limit analysis to time points within aif
    time_msk = np.logical_and(mean_tac_mskt.time >= aif.time[0],
                              mean_tac_mskt.time <= aif.time[-1])
                              
    #Apply new time mask
    if np.sum(time_msk == False) > 0:
        pet_mskt = pet_mskt[:, time_msk]
        mean_tac_mskt = Tac(mean_tac_mskt.time[time_msk],
                            mean_tac_mskt.cnt[time_msk], dc=True, h_life=h_life)
        
    #Logic for calls when we have extra images
    if img_paths is None:
    
        #Return everything we need
        return (aif, pet_hdr, pet_mskt,
        ...
        return (aif, pet_hdr, pet_mskt, msk_data, 
                msk_hdr, mean_tac_mskt, imgs, avgs)
...
def tac_plot(tac, hats=None, labels=None, title=None, out_path=None, t_hat=None):
    ...
    plt.xlabel('Time (%s)'%(tac.t_unit), color='black', weight='bold', size=10)
    plt.ylabel('Activity (%s)'%(tac.c_unit), color='black', weight='bold', size=10)
