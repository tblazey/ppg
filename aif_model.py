...
class GolishModel(AifModel):
    ...
    def pred(self, params, kernel=None):
        ...
        #Create array to store predictions
        g_pred = np.zeros(self.aif.n)

        #Ignore values where t is less than t_zero
        t_mask = self.aif.time > t_zero
        t_shift = self.aif.time[t_mask] - t_zero
        ...
        #Convolve with kernel if necessary
        if kernel is not None:
            return np.convolve(g_pred, kernel.cnt)[0:self.aif.n] * self.aif.samp
        else:
            return g_pred
...
class GolishModel(AifModel):
    ...
    def pred(self, params, kernel=None, t_new=None):
        ...
        #Convolve with kernel if necessary
        if kernel is not None:
            return np.convolve(g_pred, kernel.cnt)[0:n] * samp
        else:
            return g_pred
